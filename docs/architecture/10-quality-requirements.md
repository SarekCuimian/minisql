...
| 场景 | 刺激 | 预期结果 | 验证方式 |
|---|---|---|---|
| WAL 先行 | WAL 已追加但尚未 durable 时触发数据页 flush | 数据页不得先于对应 WAL 持久化 | 在 PageCache / LogManager test 中记录调用顺序 |
...
| 场景 | 刺激 | 预期结果 | 验证方式 |
|---|---|---|---|
| 不同页并发 load | 多线程请求不同 page number | 不因单个全局 file lock 而串行等待 I/O | PageCache concurrency benchmark |

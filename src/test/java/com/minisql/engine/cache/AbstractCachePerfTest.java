...
public class AbstractCachePerfTest {
    ...
    private static void runBenchmark(
        ...
        if (failure.get() != null) {
            fail("benchmark worker failed", failure.get());
        }
    ...
    void singleFlight_sameKey_onlyOneLoad() throws Exception {
        ...
        // load 模拟慢一些，放大并发窗口
        TestCache cache = new TestCache(/*capacity*/128, /*value*/1024, /*loadMillis*/50);
        ...
        pool.shutdownNow();
        if (failure.get() != null) {
            fail("single-flight worker failed", failure.get());
        }

        assertEquals(1, cache.loads(), "same-key concurrent miss must share one load");
    }
    ...
    void perf_allHit_qps() throws Exception {
        ...
        TestCache cache = new TestCache(/*capacity*/keySpace + 10, /*value*/256, /*loadMillis*/0);
    ...
    void perf_hotKey_singleFlight_qps_and_loads() throws Exception {
        ...
        TestCache cache = new TestCache(/*capacity*/128, /*value*/1024, /*loadMillis*/20);
    ...
    void perf_smallCapacity_churn_qps() throws Exception {
        ...
        // 容量很小：会频繁淘汰
        TestCache cache = new TestCache(/*capacity*/128, /*value*/256, /*loadMillis*/0);
    ...
    // -------------------------------
    // 5) 不同 key 并发加载：确认取消分段锁后仍可稳定处理高并发 miss
    // -------------------------------
    @Test
    @Timeout(30)
    void perf_parallelDistinctKeys_qps() throws Exception {
        int threads = 16;
        int seconds = 3;
        int keySpace = 100_000;

        TestCache cache = new TestCache(/*capacity*/10_000, /*value*/256, /*loadMillis*/0);
        ...
        runBenchmark("[PARALLEL] distinct keys", cache, threads, seconds, uniform, true);

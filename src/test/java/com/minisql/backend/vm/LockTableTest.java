...
public class LockTableTest {
    ...
    public void testLockTable() {
        ...
        assertThrows(RuntimeException.class, () -> {
            try {
                ...
                // add 抛出的 Error.DeadlockException 一般是 RuntimeException 子类
    ...
    public void testLockTable2() {
        ...
        for (long i = 1; i <= 100; i++) {
            try {
                CountDownLatch latch = lt.add(i, i);
        ...
        for (long i = 1; i <= 99; i++) {
            try {
                CountDownLatch latch = lt.add(i, i + 1);
                // 这里会进入等待队列，add 可能返回一个 CountDownLatch
        ...
        // 再次申请同样的锁应该不再触发死锁
        try {
            CountDownLatch latch = lt.add(100, 1);
    ...
    public void testWaitAndWakeupWithDelay() throws Exception {
        ...
        CountDownLatch latch1 = lt.add(1L, 1L);
        assertNull(latch1);

        // 2. T2 请求 U1，会进入等待，拿到自己的 latch
        System.out.println("[主线程] T2 尝试加锁 U1，需要等待");
        CountDownLatch waitLatch = lt.add(2L, 1L);

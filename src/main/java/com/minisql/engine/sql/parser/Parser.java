...
import com.minisql.engine.sql.ast.expression.Operand;
...
public class Parser {
    public static Statement parse(byte[] statement) throws Exception {
    ...
    private static int getAggregateIndex(Select select, String funcToken, String arg) {
        ...
        for (int i = 0; i < select.aggregates.length; i++) {
            ...
            if(agg.function == func && matchField) {
                return i;
            }
        }
        return -1;
    ...
    private static Statement parseDrop(Tokenizer tokenizer) throws Exception {
    ...
    private static Statement parseCreate(Tokenizer tokenizer) throws Exception {
        String target = tokenizer.peek();
        if(eq(target, "table")) {
            tokenizer.pop();
